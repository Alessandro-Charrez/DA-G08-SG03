...
def tarjeta_tarea(tarea):
    return rx.table.row(
        rx.table.cell(tarea["titulo"], style={"color": "black"}),  
        rx.table.cell(tarea["estado"], style={"color": "black"}),  
...
def obtener_tareas_filtradas():
    return rx.cond(
        State.mostrar_solo_pendientes,  # Usar cond para condiciones de estado
        [tarea for tarea in tareas if tarea["estado"] == "Pendiente"],  # Filtra solo tareas pendientes
        [tarea for tarea in tareas if tarea["estado"] == "Completado"]  # Filtra solo tareas completadas
    )

# Función para contar las tareas completadas o pendientes
def contar_tareas_filtradas():
    return rx.cond(
        State.mostrar_solo_pendientes,
        len([tarea for tarea in tareas if tarea["estado"] == "Pendiente"]),  # Solo cuenta tareas pendientes
        len([tarea for tarea in tareas if tarea["estado"] == "Completado"])  # Solo cuenta tareas completadas
    )
...
def index():
    return rx.box(
        ...
        rx.text(
            ...
            style={"margin-bottom": "20px", "font-size": "18px", "color": "#FFFFFF"}
        ),
